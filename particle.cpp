...
// マクロ定義
//*****************************************************************************
#define TEXTURE_MAX			(PARTICLE_LABEL_NUM)			// テクスチャの数
...
#define	WHITE_OUT			// 色減衰
#undef WHITE_OUT

#define	DIE_IF_GROUNDED		// 着地即消失
//#undef DIE_IF_GROUNDED
...
static bool						g_ParticleEnable = true;		// パーティクル有効化フラグ
...

	{
		...
		for(int nCntParticle = 0; nCntParticle < MAX_PARTICLE; nCntParticle++)
		{
			...
			{// 使用中
				...
				g_aParticle[nCntParticle].move.y -= GRAVITY;
		...
		//	// ビルボードの設定
		//	SetParticle(pos, move, XMFLOAT4(0.5f, 0.5f, 0.5f, 1.0f), fSize, fSize, nLife);
		//}
	}
}
...
int SetParticle(XMFLOAT3 pos, XMFLOAT3 scl, XMFLOAT3 move, XMFLOAT4 col, int life)
{
	...
	for (int nCntParticle = 0; nCntParticle < MAX_PARTICLE; nCntParticle++)
	{
		if (!g_aParticle[nCntParticle].bUse)
		{
			...
			g_aParticle[nCntParticle].move = move;
			g_aParticle[nCntParticle].material.Diffuse = col;
			g_aParticle[nCntParticle].nLife = life;
			g_aParticle[nCntParticle].bUse = true;
