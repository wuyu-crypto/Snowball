...
float ViewDepth(XMFLOAT3 pos) {
	...
	XMVECTOR xmvCamPos = XMLoadFloat3(&g_Camera.pos);
	XMVECTOR xmvCamAt = XMLoadFloat3(&g_Camera.at);

	// ターゲットベクトル
	XMVECTOR xmvTarget = XMLoadFloat3(&pos) - xmvCamPos;

	// 視線ベクトル
	XMVECTOR xmvView = xmvCamAt - xmvCamPos;

	// 内積
	float dot = dotProduct(&xmvTarget, &xmvView);

	// 視線の長さ
	xmvView = XMVector3Length(xmvView);
	float lengthView = 0.0f;
	XMStoreFloat(&lengthView, xmvView);

	// 内積/視線の長さ
	return dot / lengthView;
}
...
XMFLOAT3 GetTranslationFromXMMATRIX(XMMATRIX mtx) {
	...
	pos.x = mtx.r[3].m128_f32[0];
	pos.y = mtx.r[3].m128_f32[1];
	pos.z = mtx.r[3].m128_f32[2];
